...
def get_labeled_loss(
    ...
    if len(models) > 1:
        outs = torch.stack(
            [
                torch.reshape(net(net.preprocess(batch, device=device)), (-1,))
                for net in models
            ],
            dim=0,
        )
...
def MI_unlabeled_losses(outs: torch.Tensor, device="cpu"):
    ...
    for i1, md1 in enumerate(outs):
        for i2, md2 in enumerate(outs):
            if (
                i1 < i2
            ):  # we don't want to compute the mutual information of a head with itself, nor repeat the calculations
                ...
                mi_loss += torch.nn.KLDivLoss(reduction="batchmean")(
                    joint_dist.log(), marginal_dist
                )
...
def get_labeled_feature_losses(
    criterion: nn.Module, outs: torch.Tensor, hidden_labels: torch.Tensor, device="cpu"
):
...
def get_unlabeled_losses(
    ...
    if loss_type == 1:
        return MI_unlabeled_losses(outs, device=device), KL_unlabeled_losses(
            outs, device=device
        )

    # Sanity checking for baseline – uses labels
    if loss_type == 2:
        return get_labeled_feature_losses(
            criterion, outs, hidden_labels, device=device
        ), get_class_certainty_loss(criterion, outs, device=device)
