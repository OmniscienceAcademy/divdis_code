...
def train_epoch_joint(
    ...
    unlabeled,
    max_batches,
    labeled_criterion: nn.Module,
    unlabeled_criterion: nn.Module,
    optim: torch.optim.Optimizer,
    labeled_scale=1.0,
    distinct_scale=1.0,
    class_certainty_scale=1.0,
    loss_type=0,
    smooth=False,
    dynamic=False,
    device="cpu",
    ...
    cum_labeled, cum_distinct, cum_class_certainty, cum_loss = 0.0, 0.0, 0.0, 0.0
    ...
    while i < max_batches:
        labeled_batch, labels, _ = next(
            labeled
        )  # confusingly, "targets" just means "labels"
        unlabeled_batch, _, hidden_labels = next(
            unlabeled
        )  # unlab_targets is a tensor of labels for each feature

        log_img = (i + 1) % image_log_frequency == 0
        if log_img:
            print("Logging images...")
        # calculate labeled loss
        labeled_loss = get_labeled_loss(
            models, labeled_criterion, labeled_batch, labels, device=device, log=log_img
        )
        # calculate unlabeled loss
        distinct_loss, class_certainty_loss = get_unlabeled_losses(
            models,
            unlabeled_criterion,
            unlabeled_batch,
            hidden_labels,
        ...
        loss = (
            labeled_scale * labeled_loss
            + distinct_scale * distinct_loss
            + class_certainty_scale * class_certainty_loss
        )
...
class DistinctTrainer:
    def __init__(
        ...
        **kwargs,
        ...
        if lpft:
            ...
            self.lp_optim = optim(
                chain(*[model.class_heads.parameters() for model in models]),
                lr=lr[0],
                weight_decay=weight_decay,
            )
            self.ft_optim = optim(
                chain(*[model.parameters() for model in models]),
                lr=lr[1],
                weight_decay=weight_decay,
            )
        else:
        ...
        self.labeled_batch_size, self.unlabeled_batch_size = (
            container.batch_size["labeled"],
            container.batch_size["unlabeled"],
        )
    ...
    def evaluate(self, test=False):
        ...
        diag_accuracy, head_performances, subset_predictions = evaluate_unlabeled(
            self.models,
            data,
            self.n_features,
            device=self.device,
            get_subset_predictions=True,
        )
        ...
        print(
            f"worst {eval_mode} head accuracy = accuracy on the hard feature = {1-worst_head_loss}"
        )
        print(
            f" best {eval_mode} head accuracy = accuracy on the easy feature = {1-best_head_loss}"
        )
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            l_scale, d_scale, cs_scale = (
                self.labeled_scale,
                self.distinct_scale,
                self.class_certainty_scale,
            )
            ...
            loss, labeled_loss, distinct_loss, class_certainty_loss = train_epoch_joint(
                self.models,
                self.labeled,
                self.unlabeled,
                self.max_batches,
