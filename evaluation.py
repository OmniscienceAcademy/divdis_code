...
def evaluate_labeled(models: List[nn.Module], data, device="cpu"):
    ...
    counts = torch.zeros(
        [2], device=device
    )  # [number datapoints labeled 0, num labeled 1]
    corrects_by_label = 0.0  # will become tensor of shape [n_heads, 2]
    with torch.no_grad():
        for batch, labels, _ in data:
            ...
            )  # [number of 0 labels, number of 1 labels]

            corrects = (
                outs == labels
            )  # true when the prediction is correct; shape [n_heads, batchsize]
            corrects_by_label += torch.stack(
                (corrects[:, ~labels].sum(-1), corrects[:, labels].sum(-1)), dim=-1
            )  # shape [n_heads, 2 (n_classes)]
...
def evaluate_unlabeled(
    ...
    if get_subset_predictions:
        idx_sets = torch.cartesian_prod(
            *[torch.LongTensor([0, 1]).to(device) for _ in range(n_features)]
        )
    ...
    with torch.no_grad():
        for data_batch, _, targets in data:
            ...
            if len(models) > 1:
                preds = torch.stack(
                    [
                        model(model.preprocess(data_batch, device=device)).round()
                        for model in models
                    ],
                    dim=0,
                )
                ...
                preds = models[0](
                    models[0].preprocess(data_batch, device=device)
                ).round()
            ...
            if get_subset_predictions:
                for idx in idx_sets:
                    ...
                    for target in all_targets:
                        if (
                            target == idx
                        ).sum() == n_features:  # checks if target and idx are equal at every value
                            subset_size += 1
            ...
            for feature, target in enumerate(torch.t(all_targets)):
                for head, pred in enumerate(preds):
                    performances[feature][head] += (
                        (target == pred).float().sum()
                    )  # checks how good the head "head" is on feature "feature"
            ...
            for idx in idx_sets:
                ...
                if subset_size == 0:
                    ...
                    for h in range(n):
                        subset_predictions[
                            name(idx, h)
                        ] /= subset_size  # gets the average of the prediction on a subset

        performances /= total_size  # gives the mean performances
        feature_performances = torch.tensor(
            [0.0 for i in range(n_features)]
        )  # performance on each feature
...
def evaluate_saliency(
    mode, model, data, wandb_log, device="cpu", k_size=50, step_size=25
):
...
def flat_perturbation(
    ...
    with torch.no_grad():
        ...
        heatmap = (
            torch.zeros((num_classes, len(y_steps), len(x_steps))).to(device) + 0.5
        )
        num_occs = 0
        ...
        for x in x_steps:
            ...
            for y in y_steps:
                ...
                if perturbation_type == "mean":
                    occ_im[:, :, y : y + k_size, x : x + k_size] = torch.mean(
                        input[:, :, y : y + k_size, x : x + k_size],
                        axis=(-1, -2),
                        keepdims=True,
                    )
                ...
                if perturbation_type == "blur":
                    occ_im[:, :, y : y + k_size, x : x + k_size] = blur_substrate[
                        :, :, y : y + k_size, x : x + k_size
                    ]

                diff = (output - torch.cat(model(occ_im)).reshape(-1)).reshape(
                    num_classes, 1, 1
                )
    ...
    with torch.no_grad():
        ...
        while depth <= max_depth:
            ...
            if (x_cell_dim == 0) or (y_cell_dim == 0) and verbose:
                ...
                print(
                    "Depth: {}, {} x {} Cell Dim".format(depth, y_cell_dim, x_cell_dim)
                )
            possible_masks = 0
            ...
            for x in x_ixs:
                for y in y_ixs:
                    ...
                    local_saliency = (
                        F.interpolate(
                            mask, (input_y_dim, input_x_dim), mode=interp_mode
                        )
                        * saliency
                    )
            ...
            if verbose:
                print(
                    "Selected {}/{} masks at depth {}".format(
                        num_masks, possible_masks, depth
                    )
                )
            if num_masks == 0:
                depth -= 1
            ...
            while len(masks_list) > 0:
                ...
                masks = F.interpolate(
                    masks, (input_y_dim, input_x_dim), mode=interp_mode
                )

                if perturbation_type == "fade":

                    perturbed_outputs = diff_func(
                        output - torch.cat(model(input * masks)).reshape(-1)[0]
                    )

                else:
                    perturbed_outputs = diff_func(
                        output - torch.cat(model(b_imgs)).reshape(-1)[0]
                    )

                if len(list(perturbed_outputs.shape)) == 1:
                    sal = perturbed_outputs.reshape(1, -1, 1, 1) * torch.abs(masks - 1)
                else:
                    sal = perturbed_outputs.reshape(1, num_classes, 1, 1) * torch.abs(
                        masks - 1
                    )
