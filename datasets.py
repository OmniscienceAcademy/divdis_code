...
class Container:
    ...
    def __init__(self, dr,
        labeled_mix_rate=-1, unlabeled_mix_rate=0.5,
        labeled_batch_size=64, unlabeled_batch_size=64, vt_batch_size=64
        ):
        ...
        if dr is None:
            ...
            try:
                ...
                raise ValueError("Either the data path is wrong, or the data folder is missing a metadata file")
            metadata = json.load(f)
            self.classes = metadata['classes'] # list of classes for each feature
            self.n_features = metadata['n_features'] # number of features
            self.true_feat_idx = metadata['true_feat_idx'] # index of the ground truth feature
        ...
        self.mix_rate = {
            'labeled'   : labeled_mix_rate,
            'unlabeled' : unlabeled_mix_rate
        }
        self.batch_size = {
    ...
    def get_dataloader(self, luvt):
        assert luvt in ['labeled', 'unlabeled', 'val', 'test'], "Argument should be 'labeled', 'unlabeled', 'val', or 'test'"
        if luvt in self.dataloaders: return self.dataloaders[luvt]
        ...
        for dc in ['diag', 'cross']:
            ...
            for folder in folders:
                ...
                data[dc] += [(StackedImg(file), true_label, labels) for file in glob(os.path.join(folder,'*'))]

        if luvt in ['val', 'test']:
            collated_data = []
            for dc in data: collated_data += data[dc]
            drop_last = False
            ...
            if r == -1: # take all data if mix rate is -1
                ...
                collated_data = random.sample(data['diag'], n_diag) + random.sample(data['cross'], n_cross)
            else:
                raise ValueError("Mix rate must be in [0,1] or == -1")
            drop_last = True

        try: dl = DataLoader(collated_data, self.batch_size[luvt], shuffle=True, 
            collate_fn = StackedImg._collator, drop_last=drop_last)
        except:
            raise ValueError("Something went wrong while making the DataLoader, probably because " 
    ...
    def stack_from_drs(drs, classes, true_feat_idx,
        train_n, val_n, test_n, 
        train_mix_rates=[0.,0.5], val_mix_rates=[0.,0.5], test_mix_rates=[0.,0.5],
        source_batch_size=128, target_batch_size=128
        ):
        # CURRENTLY DEPRICATED TODO
        n_drs = len(drs)
        if len(classes) != n_drs: raise ValueError
        ...
        out = Container(None,
            train_mix_rates=train_mix_rates, val_mix_rates=val_mix_rates, test_mix_rates=test_mix_rates,
            source_batch_size=source_batch_size, target_batch_size=target_batch_size)
        out.classes = classes
        ...
        for tvt in ['train', 'val', 'test']:
            ...
            while len(out.data[tvt]['diag']) < n[tvt]:
                ...
                datapoint = tuple(random.choice(filelists[i][cl_id]) for i in range(n_drs))
                out.data[tvt]['diag'].append( (datapoint, cl_id, t.LongTensor([cl_id] * n_drs)) )
            ...
            while len(out.data[tvt]['cross']) < n[tvt]:
                ...
                if all(idx == cl_ids[0] for idx in cl_ids):
                    ...
                    datapoint = tuple(random.choice(filelists[i][cl_ids[i]]) for i in range(n_drs))
                    out.data[tvt]['cross'].append( (datapoint, cl_ids[true_feat_idx], t.LongTensor(cl_ids)) )
            ...
            for dc in ['diag', 'cross']:
                ...
                for filenames, _, labels in self.data[tvt][dc]:
                    ...
                    if cl_name not in folder_label_dict:
                        folder_label_dict[cl_name] = [self.classes[i][label] for i, label in enumerate(labels)]
                    if cl_name not in file_count: # if this is the first image of its class for this tvt/dc
                        os.mkdir(os.path.join(save_dr, tvt, dc, cl_name))
                        file_count[cl_name] = 0
                    else:
                        file_count[cl_name] += 1
                    img.save(os.path.join(save_dr, tvt, dc, cl_name, cl_name + '_{:04d}'.format(file_count[cl_name])) + '.png')
...
def tensorstack_image_list(imgs):
    ...
    if len(imgs) > 0:
        ...
        stacked_imgs = torch.empty(0)                               # if ims is an empty list, return an empty tensor (shape (0))
    return stacked_imgs   
...
def imgs_to_tensor_from_dir(dir, proportion_of_images=1.):          
    """
    ...
    images_to_take = round(number_of_images * proportion_of_images)     #defaults to number_of_images
...
def get_concat_v(ims: [Image.Image]) -> Image.Image:    
    ...
    max_width = 0
    for im in ims:
        current_height += im.height            # cumulatively adding up heights of all the images in the list 'ims'
        heights.append(current_height)          # the list 'heights' will be used as a guide to the horizontal levels at which these images will be pasted
        max_width = max(max_width, im.width)    # 'max_width' is updated if a "widest yet" image is newly encountered
    dst = Image.new('RGB', (max_width, current_height))   #initialises a blank RGB image 'dst' of the appropriate dimensions
    for i, im in enumerate(ims):
        dst.paste(im, (0, heights[i]))                 # pastes the images sequentially into dst at their appropriate heights
    dst = dst.convert('RGB')                           # converts resulting image into RGB mode
    return dst


def stacked_imgs_to_tensor_from_dirs(dirs, proportion_of_images=1.):     
    """
    ...
    number_of_images = min(dir_sizes)                                      # whichever of the directories in dirs has the least image files, use that many                                                                                                                                  
    images_to_take = round(number_of_images * proportion_of_images)        # 'images_to_take' defaults to 'number_of_images'
    imgpathlists = []                                                # 'imgpathlists' is going to be a list of (ususally 2) lists of imagepaths, each of which contains pathnames of all image files in one of the (usually 2) directories in dir
    height = len(dirs)                                               # this 'height' is the number of images that are going to get visuallly stacked 
    for dir in dirs:
        imgpathlists.append(directory_to_filepath_list(dir))         # add lists of imagepaths, one directory at at time 
        random.shuffle(imgpathlists[-1])                             # the last (usually second) of these image pathlists lists gets randomised
    for imnum, img_paths in enumerate(zip(*imgpathlists)):        # zip(*filelists) will be a list of lists of (usually 2) image pathnames, one from each list in 'imgpathlists', occupying corresponding positions                                                                  
        ...
        if imnum < images_to_take:                            # loop through index numbers 
            raw_images = []                                      # for each one, starting with a new empty list 'raw_images'... 
            for h in range(height):                                
                raw_images.append(Image.open(img_paths[h]))        # ...take the ordered pair from the above list with index number 'imnum',
            ...
            images.append(get_concat_v(raw_images))                # now use 'get_concat_v' to vertically stack the images in 'basic_images' and append the result to the list 'images'
    if len(images) > 0:
        stacked_imgs = torch.stack(tensorise_image_list(images))           # torch.stack concatenates a sequence of tensors along a new dimension 
